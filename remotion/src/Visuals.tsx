...
export const StatVisual: React.FC<VisualBeat> = ({text, value = 94, unit = '%'}) => {
  ...
  return <Glass style={{padding: 52, width: 760}}>
    ...
    <div style={{height: 8, background: 'rgba(255,255,255,.1)', margin: '30px 0', borderRadius: 8}}><div style={{height: '100%', width: `${n}%`, background: 'white', borderRadius: 8}} /></div>
    <div style={{fontSize: 28, lineHeight: 1.25, color: 'rgba(255,255,255,.76)'}}>{text}</div>
  </Glass>;
};

export const TimelineVisual: React.FC<VisualBeat> = ({text}) => (
  <Glass style={{padding: 50, width: 1320}}>
...
export const MapVisual: React.FC<VisualBeat> = ({text}) => {
  const frame = useCurrentFrame();
  const drift = interpolate(frame, [0, 180], [0, -40], {extrapolateRight: 'clamp'});
...
export const QuoteVisual: React.FC<VisualBeat> = ({text}) => (
  <Glass style={{padding: 65, width: 1320, borderLeft: '5px solid white'}}>
...
export const AssetVisual: React.FC<VisualBeat> = ({text, assetSrc}) => {
  ...
  return <div style={{position:'relative', width:'100%', height:'100%', overflow:'hidden'}}>
    {assetSrc ? <Img src={assetSrc} style={{width:'100%',height:'100%',objectFit:'cover',transform:`scale(${scale})`,filter:'brightness(.55) saturate(.8)'}} /> : <div style={{position:'absolute',inset:0,background:'linear-gradient(135deg,#101923,#2c3948)'}} />}
